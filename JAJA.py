...
class BulkEmailerApp:
    ...
    def create_new_profile(self, profile_name, make_active=False, initial_setup=False):
        ...
        if not initial_setup: 
            ...
            if current_active_profile_name_for_inheritance and current_active_profile_name_for_inheritance in self.profiles:
                ...
                self.log_message(f"New profile '{profile_name}' inherited SMTP & Schedule from '{current_active_profile_name_for_inheritance}'.")
    ...
    def create_tab_custom_batch(self, parent_tab):
        ...
        main_custom_frame = ttk.Frame(parent_tab)
        main_custom_frame.pack(fill=tk.BOTH, expand=True)

        # Frame for the list of custom emails
        list_frame = ttk.LabelFrame(main_custom_frame, text="Custom Email Batch", padding=10)
        list_frame.pack(pady=5, padx=5, fill=tk.BOTH, expand=True)

        self.custom_emails_listbox = tk.Listbox(list_frame, height=10, selectmode=tk.SINGLE)
        self.custom_emails_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0,5))
        
        list_scrollbar = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.custom_emails_listbox.yview)
        list_scrollbar.pack(side=tk.LEFT, fill=tk.Y)
        self.custom_emails_listbox.config(yscrollcommand=list_scrollbar.set)

        # Frame for action buttons for the list
        list_actions_frame = ttk.Frame(list_frame)
        list_actions_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(5,0))
        ...
        # Frame for sending the batch
        send_batch_frame = ttk.LabelFrame(main_custom_frame, text="Send Batch", padding=10)
        send_batch_frame.pack(pady=10, padx=5, fill=tk.X)
        
        self.custom_batch_send_button = ttk.Button(send_batch_frame, text="Send All Custom Emails in Batch", command=self.send_custom_email_batch_process, style="Accent.TButton")
        self.custom_batch_send_button.pack(pady=5, ipady=4)
        
        self.custom_batch_progress_bar = ttk.Progressbar(send_batch_frame, orient="horizontal", length=300, mode="determinate")
        self.custom_batch_progress_bar.pack(pady=5, fill=tk.X)
    ...
    def create_tab_settings_send(self, parent_tab):
        ...
        schedule_outer_frame.grid(row=1, column=0, padx=5, pady=10, sticky="ew")
        ...
        ttk.Entry(schedule_frame, textvariable=self.ui_schedule_date_var, width=12).grid(row=0, column=1, padx=5, pady=5, sticky="w") # UI specific vars
        ttk.Label(schedule_frame, text="Schedule Time (HH:MM, 24-hr):").grid(row=0, column=2, padx=(10,5), pady=5, sticky="w")
        ttk.Entry(schedule_frame, textvariable=self.ui_schedule_time_var, width=8).grid(row=0, column=3, padx=5, pady=5, sticky="w") # UI specific vars
        
        # Display active profile's default schedule
        ...
        # Informational label about overriding
        info_label_text = "(Leave UI Date/Time blank to use profile's default, or fill to override for current bulk send.)"
        info_label = ttk.Label(schedule_frame, text=info_label_text, font=("Helvetica", 8), wraplength=400) # Moved wraplength here
        info_label.grid(row=2, column=0, columnspan=4, padx=5, pady=2, sticky="w")


        action_frame = ttk.LabelFrame(parent_tab, text="Bulk Sending Actions", padding=10); action_frame.grid(row=0, column=1, padx=15, pady=5, sticky="nsew", rowspan=3) # rowspan to align
    ...
    def _auto_detect_columns(self): # Same as before
    ...
    def _load_csv_data_from_paths(self, file_paths, silent=False): # Same as before
    ...
    def load_csv_files(self): # Same as before
    ...
    def select_cv_file(self): # Same as before
    ...
    def update_column_mapping_dropdowns(self): # Same as before
    ...
    def update_column_mapping_dropdowns_state(self): # Same as before
    ...
    def _is_valid_email(self, email_string): # Same as before
    ...
    def _validate_schedule_datetime(self, date_str, time_str):
        """Validates date and time strings and returns a datetime object or None."""
        if not date_str and not time_str: return None # No schedule specified
        if not date_str or not time_str: return "Invalid" # Both must be present if one is
    ...
    def preview_email(self, manual_mode=False, custom_email_data=None): # Added custom_email_data
        ...
        if custom_email_data: # Previewing a specific custom email from the batch
            if custom_email_data["use_template"]:
                for key, placeholder_text in DEFAULT_PLACEHOLDERS.items():
                    # Use the specific placeholder value for this custom email
                    preview_fill_data[placeholder_text] = custom_email_data["template_placeholders"].get(key, f"[{key}_MISSING]")
            else: # Fully custom, no placeholders to fill from template_placeholders
        ...
        if not custom_email_data or custom_email_data["use_template"]: # Only replace if using template or not custom_email_data
    ...
    def send_emails_process(self, campaign_id_to_send=None, scheduled_campaign_data=None):
        # This method now handles both immediate and scheduled sending logic
        
        emails_to_send_list = []
        sender_email_for_campaign = self.smtp_email_var.get() # Default to active profile
        sender_password_for_campaign = self.smtp_password_var.get()
        cv_path_for_campaign = self.cv_file_path.get()

        if scheduled_campaign_data: # Sending a previously scheduled campaign
            ...
            sender_email_for_campaign = scheduled_campaign_data["sender_email"]
            sender_password_for_campaign = scheduled_campaign_data["sender_password"] # SECURITY: Be careful with stored passwords
            cv_path_for_campaign = scheduled_campaign_data["cv_path"]
            # No need to re-validate CSV, mappings etc. as they were processed at scheduling time.
        else: # Prepare for immediate or newly scheduled send
        ...
        # Scheduling UI Date/Time or Profile's default
        schedule_dt_obj = None
        date_to_use = self.ui_schedule_date_var.get()
        time_to_use = self.ui_schedule_time_var.get()

        if not date_to_use and not time_to_use: # If UI fields are blank, try profile defaults
            date_to_use = self.profile_schedule_date_var.get()
            time_to_use = self.profile_schedule_time_var.get()
            if date_to_use or time_to_use:
                self.log_message(f"Using schedule from active profile: {date_to_use} {time_to_use}")


        if date_to_use or time_to_use: # If any schedule is specified (UI or profile)
            schedule_dt_obj = self._validate_schedule_datetime(date_to_use, time_to_use)
            if schedule_dt_obj == "Invalid":
                messagebox.showerror("Error", "Invalid Schedule Date/Time format. Use YYYY-MM-DD and HH:MM or leave both blank for immediate send."); return # Corrected error message
            if schedule_dt_obj and schedule_dt_obj <= datetime.datetime.now():
                messagebox.showerror("Schedule Error", f"Scheduled date/time {schedule_dt_obj.strftime('%Y-%m-%d %H:%M')} is in the past. Choose a future time or clear to send now."); return
        
        # If we are processing a campaign that was ALREADY scheduled and its data is passed, ignore new scheduling UI
        if scheduled_campaign_data:
             self._perform_email_sending(emails_to_send_list, is_test=False) # is_test=False for actual sending
             if campaign_id_to_send in self.scheduled_campaigns:
                del self.scheduled_campaigns[campaign_id_to_send] # Remove from pending
                self.save_scheduled_campaigns_to_file()
             return


        if schedule_dt_obj: # New campaign to be scheduled
            ...
            self.scheduled_campaigns[campaign_id] = {
                ...
                "sender_email": sender_email_for_campaign,
                "sender_password": sender_password_for_campaign, # Storing password, acknowledge security implication
                "cv_path": cv_path_for_campaign,
                "status": "pending",
                "profile_name_at_schedule": self.active_profile_name.get()
            }
            self.save_scheduled_campaigns_to_file()
            self.log_message(f"Campaign ID {campaign_id} scheduled for {schedule_dt_obj.strftime('%Y-%m-%d %H:%M')}. {len(emails_to_send_list)} emails.")
            messagebox.showinfo("Scheduled", f"Email campaign scheduled for {schedule_dt_obj.strftime('%Y-%m-%d %H:%M')}.\nThe application should ideally be running at that time for automatic sending.")
    ...
    def send_manual_email_process(self): # Mostly same, uses active profile's settings
    ...
    def send_test_email_process(self): # Mostly same
    ...
    def add_or_edit_custom_email_dialog(self, edit_mode=False):
        ...
        # Placeholder vars for template mode
        ...
        def toggle_template_fields():
            ...
            if use_template_var.get():
                ...
                if not edit_mode or not existing_email_data or not existing_email_data["use_template"]:
                    if not subject_var.get() and not body_text_widget.get("1.0", tk.END).strip(): # Only clear if truly blank
                        pass # Don't clear if user was manually typing
                    elif existing_email_data and existing_email_data["use_template"]: # if switching from template to custom, clear
