I've updated the Python code to include a "Paste" button in the email section. Here is the modified code for `ASD.py`:
...
class BulkEmailerApp:
    ...
    def create_tab_email_content(self, parent_tab):
        ...
        button_frame = ttk.Frame(email_template_frame)
        ...
        # Add the Paste button to the button_frame
        ttk.Button(button_frame, text="Paste", command=self.paste_to_email_body).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Preview Email (using first CSV row if available)", command=self.preview_email).pack(side=tk.LEFT)

        email_template_frame.columnconfigure(1, weight=1)
        email_template_frame.rowconfigure(1, weight=1)

    def paste_to_email_body(self):
        try:
    ...
    def load_csv_files(self):
        ...
        if filepaths:
            self.csv_file_paths = list(filepaths); self._load_csv_data_from_paths(self.csv_file_paths)
    ...
    def select_cv_file(self):
        ...
        if file_path:
            if file_path.lower().endswith(".pdf"):
                self.cv_file_path.set(file_path); self.log_message(f"CV selected for current profile: {os.path.basename(file_path)}")
            else:
    ...
    def update_column_mapping_dropdowns_state(self):
        state = "normal" if self.csv_headers else "disabled"
