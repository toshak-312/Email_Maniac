...
class BulkEmailerApp:
    ...
    def get_default_profile_settings(self):
        return {
            ...
            "email_body": ("Dear Hiring Manager at {{COMPANY_NAME}},<br><br>"
                           "I am writing to express my keen interest in an internship opportunity, potentially in a {{ROLE}} capacity or a related field, at {{COMPANY_NAME}}.<br><br>"
                           "My name is {{FIRST_NAME}} {{LAST_NAME}}, and I am a highly motivated student with a passion for [Your Field/Area of Interest].<br><br>"
    ...
    def create_tab_email_content(self, parent_tab):
        ...
        ttk.Label(email_template_frame, text="Body (HTML is supported):").grid(row=1, column=0, padx=5, pady=5, sticky="nw")
        ...
        # Frame for action buttons below the text widget
        action_button_frame = ttk.Frame(email_template_frame)
        action_button_frame.grid(row=2, column=1, padx=5, pady=10, sticky="e")

        ttk.Button(action_button_frame, text="Paste from Clipboard", command=self.paste_from_clipboard).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(action_button_frame, text="Preview Email (using first CSV row if available)", command=self.preview_email).pack(side=tk.LEFT)

        email_template_frame.columnconfigure(1, weight=1)
        email_template_frame.rowconfigure(1, weight=1)

    def paste_from_clipboard(self):
        """Pastes text from the system clipboard into the email body widget."""
        try:
            clipboard_content = self.root.clipboard_get()
            self.email_body_text_widget.insert(tk.INSERT, clipboard_content)
            self.log_message("Pasted content from clipboard.")
        except tk.TclError:
            self.log_message("Clipboard is empty or contains no text.", error=True)
    ...
    def _perform_email_sending(self, emails_to_send_list, is_test=False, is_custom_batch=False):
        ...
        try:
            ...
            for i, email_details in enumerate(emails_to_send_list):
                ...
                # The user's content from the text box is the HTML body.
                html_body = current_body
                # Create a simple plain text fallback for email clients that don't support HTML.
                plain_text_body = "This email is in HTML format. Please use an HTML-compatible email client to view it."

                # Create a MIMEMultipart('alternative') message part for the body.
                # Attach both the plain text and HTML versions.
                # Email clients will prefer the last part added, which is the HTML version.
